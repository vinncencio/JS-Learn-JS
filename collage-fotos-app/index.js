...
const drawCollage = (images) => {
    ...
    context.canvas.width = state.currentLayout.itemWidth * state.currentLayout.columns;
    context.canvas.height = calculateGridRows(images.length) * state.currentLayout.itemHeight;
    let currentRow = 0;
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        if (i % state.currentLayout.columns === 0) {
            currentRow += 1;
            currentCanvasDx = 0;
            if (currentRow > 1) currentCanvasDy += state.currentLayout.itemHeight;
        }
        context.drawImage(
            images[i],
            0,
